...
import type { PointState } from "@/lib/backgammon/types";
    ...
    >
      <div
        ...
        style={{
          ...
          background: isDark ? "hsl(var(--primary) / 0.25)" : "hsl(var(--muted-foreground) / 0.18)",
        }}
      />
      {highlighted && (
        <div className="pointer-events-none absolute inset-1 rounded-md ring-2 ring-success/70" />
      )}
      {selected && <div className="pointer-events-none absolute inset-1 rounded-md ring-2 ring-primary" />}
      ...
      >
        ...
        {overflow > 0 && (
          <span className="z-20 text-[10px] font-semibold text-foreground">+{overflow}</span>
        )}
      </div>
    </button>
  );
